...
export async function searchBoothsByImage(
  ...
  const response = await tenantApi.postFormData(
    "/search/image/booth",
    formData
  );
...
export async function searchProductsByImage(
  ...
  if (options.minPrice)
    formData.append("minPrice", options.minPrice.toString());
  if (options.maxPrice)
    formData.append("maxPrice", options.maxPrice.toString());
...
export async function searchProductsByImageBase64(
  ...
  try {
    console.log("[ImageSearch] 开始搜索，图片长度:", imageBase64.length);

    // 确保图片为标准base64格式（检查并修正前缀）
    let base64 = imageBase64;
    if (!base64.startsWith("data:image")) {
      // 添加前缀，假定为JPEG格式（如果没有前缀）
    ...
    const requestBody = {
      ...
      boothId: options.boothId || undefined,
    };
    ...
    console.log(
      ...
      JSON.stringify({
        ...requestBody,
        image: `${base64.substring(0, 20)}...（已截断，总长度${
          base64.length
        }）`,
      })
    );
    ...
    console.log(
      "[ImageSearch] 搜索成功, 结果数量:",
      response.data.rows?.length || response.data.results?.length || 0
    );

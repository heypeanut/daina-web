...
// Mock档口管理信息
const mockBoothInfo: BoothManagementInfo = {
  id: "1",
...
// Mock API函数
const getBoothManagementInfo = async (boothId: string) => {
  // 模拟API延迟
  await new Promise(resolve => setTimeout(resolve, 1000));
  
  if (boothId === "1") {
    return mockBoothInfo;
  }
  
  throw new Error("档口不存在或无访问权限");
};

const getUserBoothStatus = async () => {
  // 模拟API延迟
  await new Promise(resolve => setTimeout(resolve, 500));
  
  return mockUserBoothStatus;
};
...
export default function BoothManagementPage() {
    ...
    const loadBoothInfo = async () => {
      try {
        ...
        if (!targetBoothId) {
          ...
          if (!activeBooth) {
            // 没有活跃档口，跳转到选择页面
            navigate("/booth/select", { replace: true });
            return;
          }

          targetBoothId = activeBooth.id;
          // 更新URL以包含档口ID
          navigate(`/booth/management?id=${targetBoothId}`, { replace: true });
        }
  ...
  const handleEditBooth = () => {
    if (boothInfo) {
      navigate(`/booth/edit?id=${boothInfo.id}`);
    }
  };

  const handleProductManagement = () => {
    if (boothInfo) {
      navigate(`/booth/products?boothId=${boothInfo.id}`);
    }
  };
      ...
      <div className="p-4 space-y-6">
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm">
          <div className="flex items-start space-x-4">
            <div className="w-24 h-24 bg-gradient-to-r overflow-hidden rounded-lg flex items-center justify-center">
              <ImageLazyLoader
        ...
        <div className="bg-white rounded-lg">
          <div className="divide-y divide-gray-100">
            ...
            <button
              onClick={() => navigate(`/booth/products/add?boothId=${boothInfo.id}`)}
              className="w-full flex items-center justify-between p-4 hover:bg-gray-50 transition-colors"
            >
              <div className="flex items-center">

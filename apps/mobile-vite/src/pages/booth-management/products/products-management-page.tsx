...
export default function ProductsManagementPage() {
  ...
  const handleBulkAction = async (action: "delete" | "on" | "off") => {
    ...
    try {
      const batchAction = action === "delete" ? "delete" : action === "on" ? "activate" : "deactivate";
          ...
          <div className="flex space-x-1 bg-gray-100 rounded-lg p-1">
            ...
            {productStatusDict?.map((statusItem) => (
              <button
                ...
                onClick={() => handleFilterChange(statusItem.value as ProductStatus)}
                className={`px-3 py-1.5 rounded-md text-sm transition-all ${
                ...
                >
                  <div className="flex p-3 gap-3">
                    ...
                    >
                      ...
                      {/* 状态标识 */}
                      <div className={`absolute -top-1 -right-1 px-1.5 py-0.5 text-xs font-medium rounded-full ${
                        product.status === "1" 
                          ? "bg-green-500 text-white shadow-sm" 
                          : "bg-gray-400 text-white shadow-sm"
                      }`}>
                        {translateDictValue(product.status, productStatusDictMap) || (product.status === "active" ? "上架" : "下架")}
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="text-xs text-gray-400">
                          更新于 {product.updatedAt ? formatDate(product.updatedAt) : '未知'}
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 safe-area-pb">
          <div className="flex gap-3">
            <button
              ...
              disabled={selectedProducts.length === 0 || batchUpdateMutation.isPending}

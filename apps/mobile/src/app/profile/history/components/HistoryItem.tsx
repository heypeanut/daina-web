...
interface HistoryItemProps {
  ...
  onRemove: (historyId: string) => Promise<void>;
...
const ItemImage: React.FC<{ footprint: Footprint; onClick: () => void }> = ({
  ...
  const altText = isProduct ? footprint.product?.name : footprint.booth?.name;
...
export const HistoryItem: React.FC<HistoryItemProps> = React.memo(
  ({ footprint, removing, onRemove, onItemClick }) => {
    const handleItemClick = () => onItemClick(footprint);
    const handleRemove = () => onRemove(footprint.historyId);

    const isRemoving = removing === footprint.historyId;
    const isProduct = footprint.type === "product";
